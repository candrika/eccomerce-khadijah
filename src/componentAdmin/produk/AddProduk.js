...
const AddProduk = () => {
  ...
  const onSubmit = async () => {
    const confirmationResult = await Swal.fire({
      title: "Apakah anda ingin menambah produk?",
      icon: "question",
    ...
    if (confirmationResult.isConfirmed) {
      try {
        const response = await axios.post(
          "https://laravel-api-10.cerise.id/api/products/create",
          {
            name: values.name,
        ...
        fetchProduct();
        const dataLength = product.length - 1;

        // console.log("response sukses", response.data.success.id);
        localStorage.setItem("id_produk", response.data.success.id);
        Swal.fire({
          title: "Success!",
        ...
        navigate(
          `/admin/produk/EditDataProduk/${localStorage.getItem("id_produk")}`
        );
