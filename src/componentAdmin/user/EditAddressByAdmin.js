  ...
  const fetchOldAddress = async () => {
    try {
      const response = await axios.get(
        `https://laravel-api-10.cerise.id/api/user/edit/address/${id}`,
        {
          headers: {
      ...
      setIdUser(response.data.address.users.id);
    ...
    if (confirmationResult.isConfirmed) {
      try {
        const response = await axios.put(
          `https://laravel-api-10.cerise.id/api/user/update/address/${id}/${idUser}`,
          {
        ...
        navigate(`/admin/user/userDetail/${idUser}`);
      } catch (error) {
...
export default EditAddressByAdm;
