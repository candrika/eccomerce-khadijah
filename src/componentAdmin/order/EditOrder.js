...
const EditOrder = () => {
  ...
  const fetchOldOrder = async () => {
    try {
      ...
      setAddPemesan([response.data.Order[0].users]);
      setAddPenerima([response.data.Order[0].penerima]);
      setInput((prevInput) => ({
        ...prevInput, // Spread the previous state
        order_date: response.data.Order[0].order_date,
        detail_alamat: response.data.Order[0].detail_alamat,
        status_pembayaran: response.data.Order[0].status_pembayaran,
        payment: response.data.Order[0].payment,
        paymentDate: response.data.Order[0].paymentDate,
        note: response.data.Order[0].note,
      }));

      const oldChannel = response.data.Order[0].channels;
      if (oldChannel.id) {
        setSelectedChannel({
          value: oldChannel.id,
          label: oldChannel.name,
        });
      }
      const oldCourier = response.data.Order[0].courier;
      if (oldCourier.id) {
        setSelectedKurir({
          value: oldChannel.id,
          label: oldChannel.name,
  ...
  const handleSubmitOrder = async () => {
    const confirmationResult = await Swal.fire({
      title: "Apakah anda ingin mengubah order ?",
      icon: "question",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
    ...
    if (confirmationResult.isConfirmed) {
      try {
        ...
        // Use the PUT method to update the order
        const response = await axios.put(
          `http://laravel-api-10.cerise.id/api/orders/update/${id}`,
          {
            send_for: send_for,
            user_id: user_id,
        ...
        setLoading(false);
        console.log("error gagal orders", error);
        Swal.fire({
          title: error.code,
          text: error.message,
